...
Status: open, app side CLOSED (attempt 100: a store mounted one frame after the first is already unproxied, and so is one mounted fifteen seconds later, read with a live in-process control - so the boundary is structural, no ordering or delay fixes it, and every app-side and fork-side repair is excluded; what remains is docs/bugs/014-webkit-report.md's work order against an instrumented WebKit build. Two instances fixed, one class-level gate; the seam has no general guard)
...
4. **An unparseable proxy string does not fail: it unproxies the store.**
   `ProxySettings.toProxyConfigurations()` in the fork builds its array by
   skipping every rule whose `toProxyConfiguration()` returns nil, and the
   caller assigns the result unconditionally. WebKit's
   `-[WKWebsiteDataStore setProxyConfigurations:]` treats nil *or an empty
   array* as `clearProxyConfigData()`, which clears `m_nwProxyConfigs` for
   that store's whole network session and calls `nw_context_clear_proxies` on
   every live context. So one malformed rule -- and the app only ever sends
   one -- turns "set this site's proxy" into "remove this site's proxy",
   with the UI still reporting the site as proxied. `toProxyConfiguration()`
   returns nil when `URL(string:)` rejects the string or `URL.host` is empty;
   the same function also defaults a missing port to 80 and rewrites a
   scheme-less `host:port` to `http://`, so a SOCKS endpoint typed without a
   scheme silently becomes a CONNECT proxy. Found by reading WebKit's source
   against the fork's (attempt 97); not yet fixed.
...
### Attempt 97 -- WebKit's own source read directly, and Onion Browser's, which uses the same API the same way

**2026-09-21**, PR #603, source only; no arm.

Attempts 93, 95 and 96 quoted WebKit through the bug report draft rather than
from the tree. Four files fetched and read end to end
(`NetworkSessionCocoa.{mm,h}`, `NetworkProcessCocoa.mm`, `WebsiteDataStore.cpp`,
plus `WKWebsiteDataStore.mm`). Three things change.

**1. The quoted snippets were accurate, and the wrapper enumeration holds.**
`forEachSessionWrapper` visits, per `SessionSet`, `sessionWithCredentialStorage`,
`ephemeralStatelessSession`, `appBoundSession->sessionWithCredentialStorage` and
each `isolatedSessions` entry's `sessionWithCredentialStorage`, across
`m_defaultSessionSet`, `m_perPageSessionSets` and `m_perParametersSessionSets`.
Every branch of `sessionWrapperForTask` returns a wrapper from that set, and
each way a new wrapper is born -- `initializeNSURLSessionsInSet`,
`SessionSet::isolatedSession`, `appBoundSession`,
`initializeEphemeralStatelessSessionIfNeeded` -- goes through
`SessionWrapper::initialize`, which calls
`applyProxyConfigurationToSessionConfiguration`. `recreateSessionWithUpdatedProxyConfigurations`
calls it too. **So the source says a later navigation should be proxied, by
every route, and the contradiction in attempt 96 is confirmed rather than
resolved.**

**2. An empty array is not a no-op, it is a clear.** From
...
and `NetworkSessionCocoa::clearProxyConfigData` empties `m_nwProxyConfigs` and
runs `nw_context_clear_proxies` over every wrapper's context. The fork's
`toProxyConfigurations()` returns `[]` when every rule fails to parse, and the
call site assigns it unconditionally, so a malformed proxy string actively
unproxies the store. Recorded as instance 4 above. This is not the frame
boundary -- every arm here sends a well-formed rule -- but it is the first
BUG-014 instance whose mechanism is named from both sides of the seam.

The same read confirms the getter is a UI-process cache
(`return _proxyConfigurations.get();`), which attempt 93 inferred.

**3. Onion Browser uses this API exactly as this app does, and its guard
cannot see the failure.** Clone read at `OnionBrowser/Browsing/Tab.swift`:
...
One store, one `WKWebView` per tab, the configuration assigned once at setup,
and no per-navigation rebuild -- `reinitWebView()` exists but is explicit.
Its fail-closed check, in `BaseNavigationDelegate`, is
`guard !webView.configuration.websiteDataStore.proxyConfigurations.isEmpty`,
which reads the UI-process cache proved above to be blind to what the network
process holds. **So it would pass in precisely the state this file describes.**

This is the closest thing to a reconciliation with the August 2026 audit that
found no main-frame bypass in this API: it removes "a Tor browser shipping on
this API would have noticed", because the shape of the bug is invisible to the
only check that browser makes. It does not confirm the bypass exists there;
nobody has run an origin-side measurement against Onion Browser.

Worth borrowing regardless: `TorManager` points an unstarted Tor at an
unroutable endpoint (`NWEndpoint.hostPort(host: .ipv4(.loopback), port: .any)`)
so a request fails rather than going direct. That is a better fail-closed
primitive than a readback for the not-yet-ready case, though it does not cover
a proxy the network process drops.

**Why it was partial.** It verifies the source the last four attempts argued
from, and it finds a real fourth instance, and it still does not name the
frame boundary's mechanism. What it removes is the last cheap hypothesis: the
wrapper enumeration is exhaustive and every construction path applies the
proxy, so nothing short of instrumentation inside the network process can say
why `m_nwProxyConfigs` fails to reach a later navigation.


### Attempt 98 -- instrument the two things 97 attempts never looked at, and measure the boundary from the near side

**2026-09-22**, PR #603; the arms have not run yet.

Attempt 97 closed the source reading: every branch of `sessionWrapperForTask`
returns a wrapper `forEachSessionWrapper` visits, and every construction path
runs `applyProxyConfigurationToSessionConfiguration`, so nothing readable says
why a later navigation is unproxied. What is left is measurement, and three of
the four things this adds are measurements nobody has taken.

**1. The network process's own lifetime.** The tier streams `log stream
--style syslog --predicate 'process == "com.apple.WebKit.Networking"'` across
the whole step and reports the distinct PIDs with the line each was first seen
on. One PID for the tier means it never died. A second one is WebKit 264307,
after which the proxy configuration is ignored -- and that reads as a run of
DIRECT verdicts rather than as a failure, which is how it has stayed invisible.
The per-timing-launch capture already in the tier cannot see this: it only
covers its own two launches.

**2. Its crash reports, unconditionally.** The capture globbed `Webspace*` and
`Runner*` and ran only when the step had already failed. Both halves were
wrong for this bug, for the same reason as (1).

**3. Stale app processes, reaped before every arm.** The runner's own job
cleanup ends by terminating an orphan `Webspace` process, so an app outliving
its arm is not hypothetical, and one still holding a proxy agent is a candidate
for what the next arm waits on (gap -2). The reap count is itself a reading:
non-zero before most arms means the stragglers are real.

**4. `integration_test/proxy_frame_ladder_test.dart`, which takes the first
slot.** Every reading in this file says frame 1 proxies and later does not, and
`proxy_timing` brackets that only from above -- a store built after the
baseline settles goes direct, and so does one built six seconds later. Six
rungs now measure the near side (frame 1, the next frame, +50ms, +150ms,
+500ms, +2s), each with its own store, upstream fixture and origin, none
waiting for an earlier rung to settle. The distinction it buys is the one that
decides what to do next: a boundary at tens of milliseconds is a race against
the network process launch that `WebsiteDataStore::setProxyConfigData`
triggers while `m_proxyConfigData` is nulled, and a race is fixable in the
fork; a boundary that bites one frame later is structural and only a WebKit
build goes further.

It takes the slot from the reassign arms, which answered their question on
both paths (94, 96) and no longer need one. The arms behind it will mostly
read nothing, which is what gap -2 costs.

**Why it is partial.** None of it has run. It also does not touch the
mechanism: three of the four are instruments and the fourth narrows a boundary
rather than explaining it. If the ladder reads `frame1=own` with every other
rung DIRECT, the app side is finished and the WebKit brief in
[014-webkit-report.md](014-webkit-report.md) is the whole remaining path.

### Attempt 99 -- the slot ARRIVES nine seconds into a process; gap -2 is not a property held from launch

**2026-09-22**, PR #603 (`29ec765`), run 35668961271, macOS job 106560942744.

The first CI the rebuilt branch had, and `proxy_binding`'s staircase is the
most informative line in this file since attempt 90:

```
[proxy-binding] verdict: containers=true,
  stair=[0ms:DIRECT 3158ms:DIRECT 6224ms:DIRECT 9287ms:proxied 9550ms:proxied],
  pair=0 of 2 proxied, raw-first=DIRECT
```

One `WKWebView`, one store, five `loadUrl` navigations to five different
origins. The 3.1 second gaps are that arm's own settle timeout, so steps 1 to
3 timed out DIRECT; steps 4 and 5 landed 263ms apart and both proxied.

**Proxy capability arrived about 9.3 seconds into the process.** The same
process's two frame-1 WebViews had none (`pair=0 of 2 proxied`) and its raw
first load was DIRECT.

**What this changes.** Gap -2 has been written as "one app process at a time
can proxy and the next one waits", i.e. a property a process either has from
birth or does not. That is wrong. A process can start without it and acquire
it while running. Every DIRECT reading taken in the first seconds of a process
is therefore ambiguous between "the platform dropped this store's proxy" and
"this process had not got the proxy yet", and that ambiguity reaches most of
the DIRECT verdicts in this file.

It also gives the reaper a mechanism to be right about. If the ~9 seconds is
the previous arm's app process finishing its exit, reaping before each arm
should collapse it, and attempt 98's `reap:` counts will say so directly.

**The frame-1 rule survives but is now under-determined.** Attempts 90, 91 and
92 each read frame-1 cells at `own` with later ones DIRECT **in a process that
already had the slot**. This run's process did not, so its ordering is the
reverse. Both can be true: the boundary attempts 90-92 measured is about when
a navigation is issued relative to its store's binding; this is about when the
process can proxy at all. Nothing so far separates them cleanly, and no arm
was built to.

**Consequence for the ladder (attempt 98).** Its rungs spanned frame 1 to 2s.
Against a capability that arrives at 9.3s every rung including its control
would have read DIRECT, and it would have reported "no slot" while sitting
directly on top of the effect. Three rungs at 5s, 10s and 15s now straddle it,
and the control changed from `frame1 == own` to "some rung reached its own
upstream" -- the old assertion would have failed the arm in exactly the case
the new rungs were added to catch.

**The rest of the run is gap -2 as usual.** `proxy_reassign kind=connect` ran
first and got `baseline=proxied`; every arm behind it read DIRECT, including
all three `proxy_matrix` positions, both `proxy_shape` launches, `proxy_rate`
(`0 of 8`), `proxy_relay_binding` and `proxy_timing` (`baseline=DIRECT`, every
other field void). The Apple job is red because five arms assert delivery and
ran without it.

**Why it was partial.** It reframes gap -2 and invalidates the "held from
launch" wording, and it does not explain what takes nine seconds. The two
candidates it does not separate are a previous app process still exiting
(testable by attempt 98's reaper) and something inside the network process
that is not ready yet (visible, now, in attempt 98's os_log capture). Both
instruments exist and neither has run.

### Attempt 100 -- ANSWERED: the boundary is structural. One frame later is already too late, and fifteen seconds does not help

**2026-09-22**, PR #603 (`7f1285d`), run 35675217780, macOS job 106580219574.

The ladder ran first, held the slot, and returned a clean reading with its
control live:

```
[proxy-ladder] frame1-proxied=true any-proxied=true
[proxy-ladder] run=first verdict: containers=true
  frame1=own next-frame=DIRECT 50ms=DIRECT 150ms=DIRECT 500ms=DIRECT
  2s=DIRECT 5s=DIRECT 10s=DIRECT 15s=DIRECT
```

Nine stores, each with its own upstream fixture and its own origin, mounted at
measured distances from the test's first frame in one process. The frame-1
store reached its own upstream. **Every other store went direct, starting with
the one mounted a single frame (~16ms) later, and staying direct out to
fifteen seconds.**

**This settles the question attempt 98 posed.** The pre-registered reading was:
a boundary at tens of milliseconds is a race against the network process
launch and the fork can fix it; a boundary that bites one frame later is
structural and only a WebKit build goes further. It bites one frame later.
**There is no window to wait for and no ordering to fix. Every app-side and
fork-side repair is now excluded**, joining bind-earlier (91), re-assign on
both `setProxyConfigData` branches (94, 96), delivery (91), store shape, store
age, view hierarchy, load mechanism, destination scheme and `allowFailover`.

What remains is [014-webkit-report.md](014-webkit-report.md)'s work order
against an instrumented build. Its Q1 is unchanged and now carries the whole
investigation.

**Attempt 99's late acquisition did not reproduce.** This run's staircase read
`stair=[0ms:DIRECT 3067ms:DIRECT 6119ms:DIRECT 9162ms:DIRECT 12222ms:DIRECT]`,
all five direct, `pair=0 of 2 proxied`. So the 9.3s acquisition in run
35668961271 stands as a single observation, not a repeatable effect, and
attempt 99's reframing of gap -2 should be read as "a process can acquire the
slot late, once observed" rather than as the rule. The ladder's far rungs were
added for it and cost nothing; they are also the evidence that waiting does
not help.

**The reaper fired zero times.** No `reap:` line appeared before any arm, so
`pgrep -x Webspace` found nothing each time: there were no stale app processes
to blame for gap -2. That candidate is dead. The runner's own cleanup did
terminate one `Webspace` at job end, which is the app of the *last* arm, not a
straggler between arms.

**Both os_log instruments reported nothing, for two defects of mine.** The
stream was backgrounded as a pipeline, so `$!` held `head`'s pid and the kill
missed `log stream` -- the runner reaped it as an orphan (`Terminate orphan
process: pid (41159) (log)`). And the report sections sat after the loop under
`bash -e`, so the non-zero exit above them skipped both. Fixed in `b57cfbf`:
one process, one pid, and the report runs from an EXIT trap.

**Confirmed at n=2.** Run 35678246338 (job 106589258529) read the ladder
again, independently: `frame1=own` with `next-frame`, `50ms`, `150ms`,
`500ms`, `2s`, `5s`, `10s` and `15s` all DIRECT, `frame1-proxied=true
any-proxied=true`. Identical to the first reading, so the boundary is not a
one-run artefact.

The os_log instruments printed nothing that run either, and the EXIT-trap fix
in `b57cfbf` was incomplete: the trap was installed at the END of the step,
after the loop, so under `bash -e` any non-zero exit before that point killed
the step without ever installing it. It could only fire in the case where it
was not needed. Moved ahead of the arms.

**Why it was partial.** It closes the app side and it does not name the
mechanism. It also leaves gap -2 where attempt 91 left it: this run's ladder
held the slot and every arm behind it read DIRECT, so the tier still yields
about one usable reading per run, and neither surviving candidate for that
(something in the network process not ready; something environmental to the
runner) has been measured. `tool/bug014/run_local.sh --slot-check 3` settles
it on real hardware in about six minutes.

### Attempt 101 -- the network process never dies; WebKit 264307 is excluded, and CI is out of cheap answers for gap -2

**2026-09-22**, PR #603 (`71b222b`), run 35681413603, macOS job 106598991762.

The os_log instruments reported for the first time, after two defects of mine
(`b57cfbf`, `d0fade8`). 18,171 lines captured, the stream killed cleanly.

**One networking process per arm, never two, and no crash reports.** Eighteen
distinct `com.apple.WebKit.Networking` pids across twenty-one arm groups --
three of those arms are pure `URLSession` and spawn none -- each first seen
inside a different arm's window:

```
59094 03:24:07  proxy_frame_ladder      81622 03:31:37  proxy_matrix (mid)
63001 03:25:30  proxy_reassign connect   86202 03:33:22  background_audio_freeze
65109 03:26:16  proxy_reassign socks5    ...
67612 03:27:03  proxy_timing (first)     23243 03:47:27  proxy_binding
```

No arm holds two. `=== network process crash reports === / networking: none`.

**So WebKit 264307 is not what gap -2 is.** A network process that crashed and
came back would show a second pid inside one arm's window and a report in
`DiagnosticReports`; neither happens. That was the last candidate this tier
could test cheaply, and it is dead alongside the stale-app-process candidate
(attempt 100's reaper, zero fires).

**The instrument's own premise was wrong and is corrected in the workflow.**
It said "one PID for the whole tier means it never died". That is false here:
the tier launches a fresh app process per arm, so one pid per arm is the
normal case and only two within one arm is the signature. The comment now says
so, because the next reader would otherwise count eighteen pids and conclude
the process is thrashing.

**The ladder reproduced a third time**, identically:
`frame1=own next-frame=DIRECT 50ms=DIRECT 150ms=DIRECT 500ms=DIRECT 2s=DIRECT
5s=DIRECT 10s=DIRECT 15s=DIRECT`, `any-proxied=true`. Attempt 100 is n=3.

**Corrected, and the ladder at n=4.** Run 35684846618 (job 106609381493) read
the ladder a fourth time, identically, and gave a second instrument reading:
18,270 lines, 18 pids across 21 arm launches, `networking: none`. But one arm
there DID hold two pids -- `offline_connection_test.dart`, 28 seconds apart --
so "no arm holds two" is too strong as a general claim and is withdrawn as
one. What survives is narrower and is what the argument actually needs: **no
PROXY arm has ever held two**, across both runs. The one that did is the arm
whose whole purpose is tearing down and restoring connectivity, where a
replaced network process is unremarkable.

The first version of this entry also under-counted because the analysis
truncated arm names to 40 characters, which collapsed `proxy_reassign
(connect)` with `(socks5)`, and the three `proxy_matrix` positions with each
other. Re-run with full names, the numbers above are what stands.

**Why it was partial, and what it means for where this goes next.** It closes
the last cheap hypothesis and names none. Gap -2 now has no candidate this CI
tier can test: not a stale app process, not a dying network process, not the
arm ordering. What is left is either environmental to GitHub's runners or
something inside the network process that only an instrumented build shows.
The first is a ten-minute experiment on real hardware --
`tool/bug014/run_local.sh --slot-check 3` -- and the second is
[014-webkit-report.md](014-webkit-report.md)'s Q1. **Another tier run buys
nothing; this is the point to move off CI.**
