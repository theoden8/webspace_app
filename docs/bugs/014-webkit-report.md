...
Status: **not filed, and now the only remaining path.** Every app-side and
fork-side repair has been measured and excluded, the last of them by the
nine-rung ladder under "The claim": a store mounted one frame after the first
is already unproxied and stays so for fifteen seconds, in a process whose
frame-1 store proxied. There is nothing left to try from outside WebKit.
...
**A navigation is proxied if and only if it is issued in the app process's
first frame.** Every navigation issued in any later frame reaches the origin
**directly**, with no error, no `didFail`, and no change to anything the
embedding application can read.

The boundary is not the store, not the `WKWebView`, and not the load. Each of
these was measured with a positive control in the same process:

| arrangement | result |
|---|---|
| four stores, four upstreams, all navigating in frame 1 | each reaches its own upstream |
| one WebView, one store, second navigation in a later frame | direct |
| WebView built **and** navigated in a later frame, its own first load | direct |
| WebView mounted in frame 1, navigated in a later frame | direct |

So multiple simultaneous proxied stores work, and nothing after the first
frame is proxied, whatever it is attached to.

**And the boundary is sharp, not a race.** Nine stores, each with its own
upstream and its own origin, mounted in one process at measured distances from
the first frame, none waiting for an earlier one to settle:
...
The frame-1 store reached its own upstream, so the process could proxy. The
store mounted a **single frame** later did not, and neither did the one
mounted fifteen seconds later. There is no window that reopens and no settling
period to wait out: whatever the first frame has, the second frame has already
lost, permanently, for the life of the process.

**This is why the first frame is the interesting number.** It is also when the
network process is launched and a store's first session is created, which is
exactly the window `WebsiteDataStore::setProxyConfigData` clears
`m_proxyConfigData` across. The correlation is what makes Q1 below worth
asking first.

It also rules out the reading a reviewer reaches for first. A configuration
applied slightly too late would produce a boundary that moves with load, and
this one does not move at all between 16ms and 15s in the same process.
...
`protect(networkProcess())` sits inside that window and launches the process
...
**4. Assigning an empty array is a clear, not a no-op.** From
...
`NetworkSessionCocoa::clearProxyConfigData` empties `m_nwProxyConfigs` and runs
`nw_context_clear_proxies` over every wrapper's context. An embedder that
builds its array by filtering out entries it could not parse therefore
un-proxies the store from a call that reads like "set the proxy". Worth
documenting even if the rest of this report is wrong.
...
non-empty in `NetworkSessionCocoa` at the moment a *later-frame* navigation
picks a session? Log its size on entry and exit of `setProxyConfigData`, and
again wherever a task is matched to a wrapper. If it is empty, the bug is the
clear in `WebsiteDataStore::setProxyConfigData` and the report writes itself;
if it is populated, the bug is in wrapper selection and the report is a
different one.

**Q1b, the same question from the other side.** The claim is a *frame*
boundary, not a store or WebView one, so the sharpest single log line is
`m_nwProxyConfigs.size()` sampled at each navigation with the frame number
alongside it. If it is non-empty in frame 1 and empty from frame 2 on, with
no `setProxyConfigData` call in between, something clears it that is not this
function, and finding that caller is the whole bug.
...
Onion Browser's source narrows the gap without closing it. It uses this API
the same way this application does -- one store and one `WKWebView` per tab,
`conf.websiteDataStore.proxyConfigurations` assigned once in `Tab.setup()`,
no rebuild per navigation -- so its arrangement is not the difference. Its
fail-closed check is
`guard !webView.configuration.websiteDataStore.proxyConfigurations.isEmpty`,
which reads the UI-process cache above. **That check passes in exactly the
state this report describes**, so "a Tor browser shipping on this API would
have noticed" is not an argument against these readings. It is not an argument
for them either: nobody has run an origin-side measurement against Onion
Browser, and that measurement is worth more than another run here.

Q1 remains the question most likely to separate the two, and this should be
