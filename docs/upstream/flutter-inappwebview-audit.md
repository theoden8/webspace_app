...
| 2767 | macOS: `responds(to: "setUpgradeKnownHostsToHTTPS:")` instead of `#available(macOS 11.3)` | We advertise Big Sur (`macos/Podfile` targets 11.0, and 11.0-11.2 is still below the API this touches), the setting defaults to `true`, and the assignment is on the path every macOS webview creation takes. An affected user crashes on first site load. The selector check is strictly stronger than the availability check, which is what fails here. |
