...
forwarding the webview's cookies and the request's `userAgent` so
authenticated downloads succeed.

The cookie read MUST be scoped to the WebView that raised
`onDownloadStartRequest`:
`inapp.CookieManager.instance().getCookies(url:, webViewController:)`.
The download is a *second* request for a URL the WebView already
fetched once, so it only carries the user's session if it reads from
the same jar that WebView uses. Under the container engine
([CONT-006](../per-site-containers/spec.md#requirement-cont-006--cookie-ops-via-containercookiemanager))
each site owns its jar and the default jar is unused, so an unscoped
read returns nothing and the re-issued `GET` goes out anonymous — the
server answers 401/403 for exactly the downloads that worked in the
page. The handler runs inside `WebViewFactory` with the native
controller and no site model in hand, so it names the WebView directly
rather than going through `ContainerCookieManager`; the fork resolves
the jar the same way for both. Enforced structurally by
[test/js/container_cookie_scope_funnel.test.js](../../../test/js/container_cookie_scope_funnel.test.js).

#### Scenario: Authenticated download
**Given** the user is logged in to a site whose download URL requires
  session cookies
**When** the download starts
**Then** the `Cookie:` header is built from
  `getCookies(url:, webViewController: <the downloading WebView>)`
...
#### Scenario: Authenticated download from a container-bound site
**Given** the container engine is active, so the site's session cookies
  live in its own `ws-<siteId>` jar and the default jar is empty
**And** the user is signed in to that site (e.g. a Google account on
  notebooklm.google.com) and triggers a download whose URL requires
  the session
**When** `onDownloadStartRequest` fires and the HTTP path reads cookies
**Then** the read is scoped to that WebView and returns the site's
  session cookies, not the empty default jar
**And** the `GET` carries them, so the server streams the file instead
  of answering 403
...
- `test/js/container_cookie_scope_funnel.test.js` — structural gate: a
  cookie op issued directly on `inapp.CookieManager.instance()` must
  pass `webViewController:`, and `_handleHttpDownload` must both take
  the downloading controller and scope its read to it. Reproducing the
  defect needs a device with the container engine and a live signed-in
  session, which no CI tier here has.
...
3. **HTTP (authenticated)** — sign in to a site that gates its
   downloads on the session (e.g. notebooklm.google.com, then download
   an Audio Overview). Expect the same flow, not `HTTP 403`. The
   `Download` log line in DevTools reports the cookie count the read
   resolved; zero there means the read missed the site's jar.
4. **Data URI** — trigger a `data:` download (e.g. a small PNG
   generated by a canvas). Expect instant save dialog.
5. **Blob** — pairdrop.net: receive a file. Expect progress + save
   dialog + the downloads sheet showing the completed task.
6. **Blob (CSP-restricted)** — open a github.com PR, trigger an attachment
