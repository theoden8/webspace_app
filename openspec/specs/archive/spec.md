...
| `cameraAllowed` (web camera access) | effectively `false` | `effectiveCameraAllowed` denies without prompting: the Allow/Block popup and Android's OS permission dialog are OS-level UI, and a grant lights the system camera indicator. Stored value preserved for when the site leaves the archive. See [web-camera-access](../web-camera-access/spec.md) CAM-006. |
