...
3. **`window.fetch` CORS fallback**: Patches `window.fetch` to fall back to `__wsFetch` on TypeError (CORS/network failures). The fallback is scoped to **cross-site** (different registrable domain, last-two-labels heuristic) **GET/HEAD** requests without `credentials: 'include'`. The bridge carries none of the WebView's cookies or request headers, so a session-bound request retried through it makes the site see an unauthenticated client and can invalidate the session (see BUG-004): same-origin (github.com) and same-site subdomains (`www.linkedin.com` → `realtime.www.linkedin.com`) must rethrow the original error untouched, a POST must not be silently converted to a GET, and an explicitly credentialed request is session-bound regardless of site.
4. **Deduplication**: Tracks loaded URLs to avoid double-loading when both `initialUserScripts` and re-injection run.
5. **`FileReader` polyfill (iOS Lockdown Mode)**: iOS Lockdown Mode strips the File API from every third-party app's WKWebViews, so page contexts lack the `FileReader` constructor (`ReferenceError: Can't find variable: FileReader`), which breaks libraries that convert fetched blobs to data URLs — DarkReader's `readResponseAsDataURL` does `new FileReader()` for every image it inlines after fetching it through `__wsFetch`. The shim installs a minimal async polyfill (`readAsDataURL` / `readAsText` / `readAsArrayBuffer` over `Blob.arrayBuffer()`/`text()`), guarded by `typeof window.FileReader === 'undefined'` so a native constructor always wins. The rest of Lockdown Mode's restrictions (IndexedDB, WebGL, Web Audio, WASM) are not polyfillable; the app surfaces a one-time notice at startup instead (`LockdownModeService`).
...
- `UserScriptEditScreen`: Form with name, optional script URL (auto-downloads on save), injection time dropdown, monospace source editor, and play button with inline console output. The enable toggle lives on the list tile only — the editor never exposes it, so a save can't silently re-enable a script the user disabled in the list. When editing an existing script, the script's `id` and `enabled` flag are preserved verbatim.
