...
- [x] 5.5 The artwork fetch in `media_session_service.dart` takes a page-supplied URL and had only the literal half; both seams now go through `classifyOutboundTarget`, and the range table has one home instead of two copies.
