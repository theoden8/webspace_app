...
- **The app declares the capability**, deliberately and in one place per
  ...
  `audio-input` entitlement, plus a `MicrophonePermissionService` twin of
  `CameraPermissionService` for the Android runtime permission. The
  `os_capability_declarations` allowlists widen by explicit edit, and the
  test's header comment stops using the microphone as its worked example of a
  capability the app does not hold.
...
- **Combined audio+video keeps working in every mode pairing.** The shim's
  split stays for mixed pairings (real audio plus simulated video is served as
  a platform audio-only request combined with a canvas video track), and the
  one case that cannot be split is handled where it arises: iOS and macOS
  report a single `CAMERA_AND_MICROPHONE` resource that cannot be half-granted,
  so it is granted only when both modes are `real` and denied otherwise.
...
- Affected code: `lib/settings/microphone.dart`,
  ...
  `lib/services/microphone_permission_service.dart` (new),
  `lib/services/webview.dart`, `lib/web_view_model.dart`, `lib/main.dart`,
  `lib/screens/inappbrowser.dart`, `lib/screens/site_permissions.dart`,
  `lib/widgets/site_permission_badges.dart`, `lib/l10n/app_*.arb`.
- Affected native: `android/app/src/main/AndroidManifest.xml`, a
  `MicrophonePermissionPlugin.kt` beside `CameraPermissionPlugin.kt`,
...
- Affected gates: `test/js/os_capability_declarations.test.js` (allowlists and
  header), `test/js/camera_capture_stop_funnel.test.js` (renamed hook),
