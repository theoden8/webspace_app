...
- [x] 6.2e Conflux off while a pin is in force: the real-tor tier showed `{de}` leaving through the Netherlands and `{us}` through Germany. Closing a conflux leg relaunches it with its set's pre-pin exit, and streams take any linked set whose exit is not excluded, so the pin sets `ConfluxEnabled 0` in the same SETCONF as `ExitNodes` and clearing it restores `auto`. Gated by `test/js/tor_exit_pin_conflux.test.js`.
