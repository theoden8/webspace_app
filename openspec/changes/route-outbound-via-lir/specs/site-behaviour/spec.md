...
Both rows SHALL be shown only while developer mode is on (DEVTOOLS-010), the same gate LIR-014 puts on routing itself. While it is off they are hidden, their stored values are kept, and the Behaviour row's summary SHALL NOT name the switch.
...
The Behaviour row's summary (BEHAV-002) SHALL name the routing switch when it is on and developer mode is on, like any other switch.

#### Scenario: Developer mode off hides the routing rows
...
- **GIVEN** the Behaviour screen is open and developer mode is on
