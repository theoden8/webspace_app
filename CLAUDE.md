...
| icon-fetching | progressive favicon w/ fallbacks; on Android the icon the site's own webview reports (`onReceivedIcon`) wins when it is the page's own, >= 32px, and not a badge swapped in after load; the fdroid build never asks Google or DuckDuckGo |
