...
- Language identity (file actually written in its claimed language, not left in English or swapped) is enforced by [test/js/l10n_language_test.js](test/js/l10n_language_test.js) (runs under `npm run test:js`, no VRAM). Non-Latin locales pass a Unicode-script-coverage check; Latin locales use tinyld n-gram detection. A **new locale MUST be classified** in [test/js/helpers/l10n_language.js](test/js/helpers/l10n_language.js) (`SCRIPTS`, `LATIN`, or `WEAK`) or the test fails. For ad-hoc review of individual suspect strings: `node tool/check_l10n_language.js --per-string <locale>` (per-string LID is intentionally noisy — a review queue, not pass/fail).
