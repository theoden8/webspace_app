...
- Every key MUST carry a non-empty `description` (enforced by [test/l10n_coverage_test.dart](test/l10n_coverage_test.dart)) — that description is the context a translator/general model uses, so write it for someone who can't see the screen.
- To add a locale: hand `app_en.arb` (values + descriptions) to any general-purpose model, ask it to translate the values keeping `{placeholder}` tokens verbatim, save as `app_<locale>.arb`. No committed script or API key. Coverage (key + placeholder parity, no empties) is enforced by [test/l10n_coverage_test.dart](test/l10n_coverage_test.dart).
- Language identity (file actually written in its claimed language, not left in English or swapped) is enforced by [test/js/l10n_language_test.js](test/js/l10n_language_test.js) (runs under `npm run test:js`, no VRAM). Detection uses CLD3 via `cld3-asm` (pure WASM, no native build/model download) for every locale, Latin and non-Latin alike. Logic + the three benign code aliases (`he`→`iw`, `nb`→`no`, `bs`↔`hr`) live in [test/js/helpers/l10n_language.js](test/js/helpers/l10n_language.js); a new locale needs an `ACCEPT` entry only if CLD3 reports a code other than its region-stripped stem. For ad-hoc review of individual suspect strings: `node tool/check_l10n_language.js --per-string <locale>` (per-string detection is a review queue, not pass/fail).
