...
(function(){
  ...
  /// Create every proxied site's container store, with its proxy on it, in
  /// one native call — before anything else in the process has touched
  /// WebKit's network process (LEAK-003, BUG-014).
  ///
  /// On iOS and macOS a store's proxy reaches the network process either in
  /// the parameters that create its network session or as an update
  /// afterwards, and WebKit clears the pending proxy before the call that
  /// registers the session. So the assignment that registers a store can
  /// never carry the proxy into that store's session parameters, and the
  /// update path does not take: only stores already armed when the network
  /// process comes up are proxied. Arming one store per WebView — which is
  /// what building a WebView does — therefore proxies whichever site is
  /// opened first in a launch and leaves every later one on the device IP.
  ///
  /// Two constraints the caller has to keep:
  ///
  ///  * **One call.** Each container in its own call is one run-loop turn
  ///    each, and all but the first miss the window. The batch is why
  ///    [`inapp.ContainerController.prepareContainers`] takes a list.
  ///  * **First.** It must run before any cookie read, any WebView, and
  ///    before [ProxyRouterService] — any of those registers a session and
  ///    closes the window.
  ///
  /// Sites whose proxy is not resolvable yet are skipped, not guessed at: a
  /// Tor site whose runtime has not reported its SOCKS port has no address
  /// to arm, and arming the wrong one would be a leak rather than a gap.
  /// Those keep the old behaviour (bound only if opened first) until the
  /// port is known before startup.
  ///
  /// Returns how many stores were armed. A no-op off iOS/macOS: Android's
  /// `ProxyController` is process-wide and Linux sets the proxy on the
  /// session itself, so neither has the ordering constraint.
  static Future<int> prearmProxiedContainers(
    List<
        ({
          String? siteId,
          String? archiveContainerId,
    ...
    if (!hostIsIOS && !hostIsMacOS) return 0;
    if (!ContainerNative.instance.cachedSupported) return 0;
    final specs = <inapp.ContainerProxySpec>[];
    final skipped = <String>[];
    for (final site in sites) {
      final binding = resolveStoreBinding(
        siteId: site.siteId,
        archiveContainerId: site.archiveContainerId,
        incognito: site.incognito,
        proxySettings: site.proxySettings,
      );
      final containerId = binding.containerId;
      if (containerId == null) continue;
      if (binding.proxy == null) {
        // `proxyUnavailable` means the site wants a proxy and none could be
        // resolved. The WebView blanks its load rather than leaking, so
        // there is nothing to arm -- but it is worth saying which sites are
        // in that state, because on this path it is usually a Tor runtime
        // that has not come up yet rather than a malformed address.
        if (binding.proxyUnavailable) skipped.add(containerId);
        continue;
      }
      specs.add(inapp.ContainerProxySpec(
        containerId: containerId,
        proxySettings: binding.proxy!.toMap(),
      ));
    }
    if (specs.isEmpty && skipped.isEmpty) return 0;
    try {
      final armed =
          await inapp.ContainerController.instance().prepareContainers(specs);
      LogService.instance.log(
        'Proxy',
        'Pre-armed $armed of ${specs.length} container store(s) before the '
            'network process'
            '${skipped.isEmpty ? '' : '; ${skipped.length} site(s) had no '
                'resolvable proxy yet and keep the device IP until restart'}',
        level: LogLevel.info,
        sensitivity: LogSensitivity.sensitive,
      );
      return armed;
      ...
      LogService.instance.log(
        'Proxy',
        'Pre-arming container proxies failed: $e',
        level: LogLevel.error,
        sensitivity: LogSensitivity.sensitive,
      );
      return 0;
