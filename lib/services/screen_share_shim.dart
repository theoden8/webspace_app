...
// mode to fall through to, on any platform.
//
// Why there is no real mode. A display capture is whole-surface by
// construction. On Android the only route is MediaProjection, which mirrors
// the entire device; on every platform this app's own window holds the drawer,
// the tab strip, and whichever other site the user switches to. So a site
// granted a real screen would be watching every OTHER site in the webspace —
// the one thing per-site isolation exists to prevent. A user-picked file is
// the only surface that can be handed over and still be only the surface the
// user meant to hand over.
//
// The per-site decision lives in Dart (`WebViewModel.screenShareMode`); this
// shim asks for it through the `webScreenShareRequest` handler on the first
// request, so the popup, the remembered choice, the backgrounded-site gate and
// the archive-tier override are all enforced in one place rather than
// duplicated in JS.
//
...
//
// Must be injected at DOCUMENT_START so it beats the page's own capture code.
...
/// `label`, matching what a real display capture reports.
///
